  ...
  <div>
    <div>
      ...
      <el-button-group class="toolBox">
        <el-button size="small" v-for="(item,index) in buttonGroups" :key="index" @click="dynamicMethod(item.method)" :icon="item.icon">{{item.label}}
        </el-button>
      </el-button-group>
    </div>
    <!--数据表格-->
    <el-row>
      <el-col :span="24">
        <el-table :data="tableData" border stripe highlight-current-row :height="tableHeight" style="width: 100%" @row-click="handleRowClick"
          ref="multipleTable">
          ...
          <template v-for="(el,i) in tableColumns">
            <el-table-column :label="el.label" header-align="center" v-if="el.show " :width="el.width || ''" :key="el.prop" :fixed="el.fixed"
              :prop="el.prop" show-overflow-tooltip>
            </el-table-column>
          </template>
          <el-table-column fixed="right" label="操作" width="200">
            <template slot-scope="scope">
              <el-button type="text" @click="handleAuthClick(scope.row)" size="small">赋权</el-button>
    ...
    <!--列自定义-->
    <CustomTableCols :defaultCols="defaultColumns" customName="sysrole" @changeColumns="changeColumns" />
    <!--查询条件-->
