<template>
  <div>
    ...
    <!--新增编辑页面-->
    <CustomForm :show.sync="showForm" title="用户编辑" :control="formControl" :model="formField" :rules="formRules" @ok="saveForm"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      showForm: false,
      formField: { id: '', loginName: '', nickName: '', passWord: '', cmfPassWord: '', mobilePhone: '', qq: '', email: '', isEnable: '', employeId: '' },
      formControl: [
  ...
  methods: {
    ...
    addAndEdit() {
      ...
      this.showForm = true
      this.formControl[8].options = this.optionsArray
    },
    /**
     * 是否启用/禁用用户
     */
   ...
   */
    ...
    handleEditClick(row) {
      ...
      this.formField.employeId = row.employeId
      this.formControl[8].options = this.optionsArray
    },
    /**
     * 授权
