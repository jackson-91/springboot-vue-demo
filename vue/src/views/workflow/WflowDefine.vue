<template>
  <div>
    ...
    <!--新增编辑页面-->
    <CustomForm :show.sync="showForm" title="流程定义编辑" :control="wflowDefineControl" :model="wflowDefineForm" :rules="wflowDefineRules" @ok="saveForm"
...
export default {
  ...
  data() {
    return {
      ...
      showForm: false,
      wflowDefineForm: {
        id: "", name: "", key: "", category: "", description: "", bpmnfile: { fileList: [] }, pngfile: { fileList: [] },
      },
      wflowDefineControl: [
  ...
  methods: {
    ...
    startProcess() {
      ...
      this.$http
        .get("/api/wflowDefine/start-process-test", {
          params: {
            definitionId: this.$refs.multipleTable.selection[0].id,
            processKey: this.$refs.multipleTable.selection[0].key,
          },
        })
        .then((res) => {
          if (res.code == "0") {
    ...
    saveForm(from) {
      ...
      this.$http
        .post("/api/wflowDefine/deploy", this.wflowDefineForm)
        .then((res) => {
          if (res.code == "0") {
            this.$message.success(res.msg);
            this.searchData();
